...
### 허용 (OSPO 확인 불필요)
- 내부 업무 자동화 (번역, 요약 등)
- 고객 상담 챗봇 (차별/허위정보 방지 조치 포함)
- 콘텐츠 생성 도구 (사람의 검토 포함)

### 검토 필요 (OSPO 문의)
- B2B LLM API 서비스 제공
- 파인튜닝 모델 재배포
- 고위험 분야 적용 (의료, 금융, 법률, 채용)
...
BigScience RAIL 모델 사용 전:

- [ ] 사용 목적이 금지 용도에 해당하지 않는지 확인
- [ ] 고위험 의사결정에 사용하지 않는지 확인
- [ ] AI 생성물에 대한 사람의 검토 프로세스 마련
- [ ] 사용자의 악용 방지 조치 구현
- [ ] 파생 모델 배포 시 모델 카드 준비
- [ ] 불확실한 경우 OSPO에 문의

## BigScience RAIL vs 다른 라이선스

| 구분 | BigScience RAIL | CreativeML RAIL | Llama 2 |
|-----|----------------|-----------------|---------|
| 대상 | 언어 모델 | 이미지 모델 | 언어 모델 |
| 상업적 사용 | 허용 | 허용 | 조건부 |
| 용도 제한 | 포괄적 | 포괄적 | 포괄적 + 규모 제한 |
| 파생 모델 | 제한 전파 | 제한 전파 | 제한 전파 |
