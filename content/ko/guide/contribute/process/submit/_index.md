...
기여할 파일이 모두 준비되었다면, Pull Request로 기여를 제출하라.
...
## 4. Feedback을 받아라
기여를 제출하면 프로젝트로부터 Feedback을 받는다. 일반적으로 Feedback은 개선이나 변경 사항이 어떤 방식으로 동작하는지, 왜 그런 방식을 채택했는지 등에 대한 설명을 요구한다. 이 Feedback은 때로는 대응하기 어려울 수도 있지만, 기여의 수준을 높이는 과정이라고 받아들이는 것이 좋다.
...
‌PR을 오픈한 상태로 응답 없이 남겨두는 것은 결례이다. 더 이상 문제를 해결할 여건이 아니라면 Maintainer에게 더 진행할 수 없다고 알려라. 그러면 PR을 Close 하거나 다른 사람이 넘겨받아 진행할 수도 있다.
