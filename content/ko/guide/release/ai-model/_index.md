...
| 규제 | 수출 통제(ECCN) | 여기에 더해 EU 인공지능법과 국내 인공지능 기본법의 문서화 요구 |
...
1. 소속 조직 내부 승인을 받고 OSRB에 검토를 요청합니다([공개 절차](../process/)의 A 단계).
...
비공개 저장소 상태에서 미리 점검하실 수 있습니다. 모델을 비공개로 올린 뒤 읽기 권한을 OSRB에
공유하시면 빠진 항목과 채우는 방법을 정리해 드립니다. 권한은 계정 전체가 열리는 토큰 대신
해당 저장소 하나로 범위를 좁힌 토큰으로 주시면 됩니다.

조직 계정에 토큰 승인 정책이 걸려 있으면 관리자 승인 전까지 대기 상태가 되므로, 공개 일정이
정해져 있다면 토큰 발급을 먼저 진행하시기 바랍니다.
