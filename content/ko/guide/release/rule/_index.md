...
SK텔레콤은 오픈소스 커뮤니티와의 협업 가치를 존중하며, 내부 소프트웨어를 오픈소스 프로젝트로 공개하는 것을 장려한다. 하지만 SK텔레콤의 지식재산을 보호하고 의도치 않은 저작권 침해를 방지하기 위해 준수해야 할 몇 가지 규칙이 있다. 
...
## 승인을 받아라
오픈소스 공개는 저작권 관점에서 저작자가 저작물을 누구나 수정/사용/배포할 수 있는 권한을 오픈소스 라이선스를 통해 부여하는 것이다. 일반적으로 고용 기간에 만든 저작물의 저작권은 고용주가 소유한다. 즉, SK텔레콤 구성원이 만든 저작물은 SK텔레콤이 소유한다. 구성원이 임의로 저작물을 오픈소스로 공개하면 불필요한 저작권 침해 이슈가 생길 수 있다. 
...
오픈소스 프로젝트에서 발생할 수 있는 최악의 상황 중 하나는 법적으로 문제가 있는 코드가 프로젝트에 포함되는 것이다. 회사가 배포할 권리가 없는 코드이거나, 다른 회사의 특허와 같은 IP를 침해하는 코드가 법적인 문제를 일으킬 수 있다. 따라서, 공개할 코드를 준비하면서 모든 코드의 출처를 확인하고 문제 소지가 있는 코드는 삭제해야 한다.‌ 
...
* 오픈소스 커뮤니티에서 이미 해결한 문제를 다루는 코드는 공개하지 않는다. 이런 경우라면 기존의 오픈소스 프로젝트에 기여하는 것이 좋다.
