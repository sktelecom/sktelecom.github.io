...
```mermaid
graph TD
    A[공급 소프트웨어 분류] --> Q{어떤 유형인가?}
    Q -->|애플리케이션| S1[소스코드 스캔<br>cdxgen 또는 BomLens]
    Q -->|OS 포함| S2[앱 층: 소스코드 스캔 cdxgen 또는 BomLens<br>OS 층: 납품 이미지나 rootfs 스캔 Syft 또는 Trivy<br>두 층을 합쳐 제출]
    Q -->|순수 바이너리| S3[바이너리 분석 + 업스트림 SBOM 요청이나 수동 기재<br>SKT의 BDBA로 보완]
    S1 --> P[SBOM 제출]
    S2 --> P
    S3 --> P
```

납품물 유형에 따라 스캔 대상과 도구가 달라지지만, 공통 원칙은 하나입니다. 공급사가 개발한 코드는 완성된 산출물이 아니라 소스코드를 스캔합니다. 소스를 두고 펌웨어나 바이너리를 직접 스캔하면 패키지 매니저 메타데이터가 없어 purl이 누락되고, 취약점 매칭이 전량 실패해 반려됩니다.

애플리케이션(소스코드로 개발한 앱, 직접 빌드한 실행 파일, OS 없는 펌웨어)은 소스코드를 cdxgen 또는 [BomLens](../skt-scanner/)로 스캔합니다. 전이 의존성까지 정확히 잡으려면 이 방식이 기준입니다.
...
소스가 없는 서드파티나 재판매 바이너리는 소스코드를 스캔할 수 없습니다. 도구가 식별할 수 있는 만큼 바이너리를 분석하고, 나머지는 업스트림에 SBOM을 요청하거나 아는 구성요소를 직접 기재합니다. 정밀 식별은 SK텔레콤이 바이너리 구성 분석(BDBA)으로 보완합니다.
