...
graph TD
    ...
    D -- No --> C[소스코드 대상 cdxgen 권장<br>펌웨어·바이너리도 소스코드를 스캔]
```

서버처럼 OS와 애플리케이션이 결합된 경우는 한 번의 스캔으로 끝나지 않습니다. 층별로 나눠 생성하는 전체 절차는 [서버 SBOM 생성](../server-delivery/)을 참고하세요.

펌웨어나 바이너리를 납품하는 경우에도 완성된 산출물을 직접 스캔하지 말고, 이를 빌드한 소스코드를 대상으로 SBOM을 생성해야 합니다. 펌웨어나 바이너리를 그대로 스캔하면 패키지 매니저 메타데이터가 없어 purl이 누락되고, 그 결과 취약점 매칭이 실패해 SBOM이 반려됩니다.
