...
- [ ] Maven·Gradle 프로젝트를 Syft로만 스캔하지 않았는가? Syft는 `pom.xml`이나 빌드 스크립트의 직접 선언만 읽어 전이 의존성이 빠집니다. cdxgen이나 언어별 CycloneDX 플러그인을 사용하시기 바랍니다.
- [ ] npm 개발 의존성을 포함해야 하는데 빠지지 않았는가? Syft는 기본값으로 제외하므로 `SYFT_JAVASCRIPT_INCLUDE_DEV_DEPENDENCIES=true`를 지정해야 포함됩니다.
- [ ] 서버 납품인 경우 OS 패키지가 포함되어 있는가? 애플리케이션 소스만 스캔하면 설치된 rpm/dpkg 패키지가 통째로 빠집니다. 절차는 [SBOM 생성 방법](../creation-guide/#서버-납품)의 서버 납품 절을 참고하세요.
...
| 검사 항목 | 체크리스트 대응 |
|-----------|-----------------|
| 스펙 버전 범위 (CycloneDX 1.3~1.7, SPDX 2.2~2.3) | 2. 필수 데이터 필드 |
| 생성 일시, 생성 도구, 최상위 컴포넌트 이름·버전 | 2. 필수 데이터 필드 |
| 모든 컴포넌트의 이름·버전 | 2. 필수 데이터 필드 |
| 직접·전이적 의존성 포함 여부 | 3. 의존성 완전성 확인 |
| PURL 보유율, 표준 형식(`pkg:type/name@version`), `pkg:generic` 금지, 규격에 정의된 타입, 네임스페이스 필수 타입의 네임스페이스 | 4. 식별자 (PURL) 확인 |
| 라이선스·해시 보유율 (권장 항목) | — |

위 표는 다음 릴리스의 BomLens 기준입니다. 현재 배포된 v1.12.0의 자동 검증은 CycloneDX 1.7을 지원 범위 밖으로 표시하고, 네임스페이스 검사도 OS 패키지(rpm, deb, apk)까지만 수행하며, 규격에 없는 타입은 따로 잡지 않습니다. v1.12.0으로 검증한다면 CycloneDX 1.7로 생성했거나 maven 등 다른 타입을 쓰는 경우에 위 jq 명령으로 함께 확인하시기 바랍니다.
...
제출 전에는 위 명령에 `--conformance-profile skt-submission`을 추가해서, SK텔레콤 심사와 같은 기준(PURL 포함률 100%, `pkg:generic` 식별자 없음)으로 확인하시기 바랍니다. 웹 UI의 제출 검토 화면은 이 기준을 이미 기본값으로 적용하므로, 이 옵션은 CLI를 직접 실행할 때만 필요합니다.
