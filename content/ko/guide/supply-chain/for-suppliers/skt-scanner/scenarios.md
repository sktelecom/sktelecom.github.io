...
CLI가 익숙하지 않으면 `./scan-sbom.sh --ui`로 브라우저 UI를 실행한 뒤, **스캔 대상**(현재 폴더 / GitHub URL / ZIP 업로드 / SBOM 업로드 / 펌웨어 업로드 / Docker 이미지)을 선택해 동일하게 처리할 수 있습니다. 완료 후 3종 산출물을 화면에서 보거나 내려받습니다.
