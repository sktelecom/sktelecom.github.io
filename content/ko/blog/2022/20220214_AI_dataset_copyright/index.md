...
여기서 잠깐 GitHub Copilot과 관련한 논쟁에 대해 언급하고 넘어가겠습니다. 최근 미국의 SFC<sub>Software Freedom Conversancy</sub>에서는 "[If Software is My Copilot, Who Programmed My Software?](https://sfconservancy.org/blog/2022/feb/03/github-copilot-copyleft-gpl/)"라는 글을 게재하여 Microsoft와 GitHub의 주장을 반박하였습니다. 
...
2. ML system이 만든 output은 시스템 operator에게 속한다<sub>the output belongs to the operator</sub>.
{{% /alert %}}
...
- 라이선스 조건이 존중되지 않는 경우, 즉 라이선스가 부여하지 않은 권리를 data가 행사하거나 의무가 이행되지 않는 경우 (잠재적) 계약 위반 또는 계약 위반에 해당할 수 있다.
{{% /alert %}}
...
- [Montreal Data License (MDL)](https://arxiv.org/abs/1903.12262) 형식을 보완하여 사용할 것을 제안하며, 이를 Enhanced MDL이라 부른다.
...
논문에서는 위와 같은 방식으로 다른 Dataset도 Case Study를 진행하였습니다. 그 내용을 살펴보겠습니다. 
...
그럼 이 논문에서 여섯 개의 dataset을 연구한 결과를 살펴보겠습니다.
...
- 이 연구에서는 이미지 Dataset만 다루었다. video, text 등 다른 Dataset에서는 또 다른 문제가 제기될 수 있기에 추가 연구가 필요하다.
